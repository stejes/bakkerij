...
class UserDAO {
    ...
    /*public function getByEmailAndPass($email, $password) {
        $sql = "select id, name, firstname, address, cityId, email, password, isBlocked from customers where email = :email and password = :password";
        $dbh = new PDO(DBConfig::$DB_CONNSTRING, DBConfig::$DB_USERNAME, DBConfig::$DB_PASSWORD);
        $stmt = $dbh->prepare($sql);
    ...
    public function update($user){
        $sql = "update customers set name = :name, firstname = :firstname, address  = :address, cityId  =:cityId, isBlocked = :isBlocked, password = :password where id = :id";
        $dbh = new PDO(DBConfig::$DB_CONNSTRING, DBConfig::$DB_USERNAME, DBConfig::$DB_PASSWORD);
        $stmt = $dbh->prepare($sql);
        $stmt->execute(array(":name" => $user->getName(), ":firstname" => $user->getFirstname(), ":address" => $user->getAddress(),
            ":cityId" => $user->getCity()->getId(), ":id" => $user->getId(), ":isBlocked"=> $user->getIsBlocked(), ":password" => $user->getPassword()));
        $dbh = null;
    }
